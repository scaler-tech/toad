...
- Linear agent (polled): when connected as a Linear app (`toad linear connect`), a poller (`internal/linearagent`, `linear_agent.poll_seconds`, default 15s) picks up agent sessions created by @-mentioning or delegating to toad in Linear; the OAuth app must have webhooks ENABLED with agent session events (a dead/dummy endpoint is fine — Linear won't create agent sessions without webhooks on, but toad only ever polls), acks with a `thought` activity, runs the standard read-only investigation (reusing stored findings when fresh), and replies with a `response` activity — never filing tickets or mutating issues from this path; delivery is at-least-once — a crash between response and handled-record write repeats the answer rather than losing it
